 ...
 */function ut(t){return pt({...t,state:!0,attribute:!1})}let ft=class extends at{constructor(){super(...arguments),this.language="en",this._now=Date.now()}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=Date.now()},1e3)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(window.clearInterval(this._interval),this._interval=void 0)}_format(t){const e="nl"===this.language;if(t<=0)return e?"vertrokken":"departed";const i=Math.floor(t/3600),s=Math.floor(t%3600/60),o=e?"over":"in";return i>0?`${o} ${i}${e?"u":"h"} ${s}m`:`${o} ${s} min`}render(){if(!this.arrival)return W``;const t=new Date(this.arrival).getTime();if(Number.isNaN(t))return W``;const e=Math.round((t-this._now)/1e3);return W`<span class="countdown ${e<=0?"departed":""}"
  ...
  `,t([pt({attribute:!1})],ft.prototype,"arrival",void 0),t([pt({attribute:!1})],ft.prototype,"language",void 0),t([ut()],ft.prototype,"_now",void 0),ft=t([ct("google-transit-countdown")],ft);const gt={BUS:"mdi:bus",TRAM:"mdi:tram",SUBWAY:"mdi:subway",HEAVY_RAIL:"mdi:train",RAIL:"mdi:train",COMMUTER_TRAIN:"mdi:train",LIGHT_RAIL:"mdi:tram",FERRY:"mdi:ferry",WALK:"mdi:walk"};function mt(t){const e=Math.floor(t%60),i=Math.floor(t/60),s=i%60,o=Math.floor(i/60),n=e>0?`${e}s`:"",r=0===e?"min":"m";return o>0?`${o}h${s}${r}${n}`:s>0?`${s}${r}${n}`:n||"0s"}const _t=new Set(["HEAVY_RAIL","RAIL","COMMUTER_TRAIN"]);function $t(t){if(!t.length)return t;const e=t[t.length-1];return"WALK"===e.mode&&(e.duration||0)<60?t.slice(0,-1):t}let vt=class extends at{constructor(){super(...arguments),this.legs=[],this.expanded=!1,this.language="en"}render(){if(!this.legs?.length)return W``;const t=$t(this.legs);if(!t.length)return W``;const e=t.map(t=>this._spanSeconds(t)),i=e.reduce((t,e)=>t+e,0)||1,s=e.map(t=>Math.max(t/i*100,6)),o=s.map(t=>`minmax(0, ${t}fr)`).join(" ");return W`
      <div class="bar" style="grid-template-columns: ${o}">
        ${t.map((t,i)=>{const s="WALK"===t.mode,o=gt[t.mode]??"mdi:map-marker-path",n=t.line_color||"var(--secondary-text-color, #727272)",r=Math.min(t.duration||0,e[i]),a=e[i]-r;return W`
            <div
              class="segment ${s?"walk":"transit"}"
              style=${s?"":`background: ${n};`}
              title=${t.line_full_name||t.mode}
            >
              ${s?W`
                    <div
                      class="walk-part"
                  ...
                  `:W`<ha-icon icon=${o}></ha-icon
                    ><span class="line-name">${t.line_name}</span>`}
            </div>
          `})}
      </div>
      <div class="times" style="grid-template-columns: ${o}">
        ${t.map((t,i)=>{const s="WALK"===t.mode?Math.min(t.duration||0,e[i]):e[i];return W`<span>${this._legTimeLabel(t,s)}</span>`})}
      </div>
      ${this.expanded?W`<div class="leg-details">
            ${t.map(t=>{const e=this._legDetailLabel(t);return e?W`<div class="leg-detail-row">
                    <ha-icon
    ...
    `}_legTimeLabel(t,e){return t.departure_time_local?e>0?`${t.departure_time_local} (${mt(e)})`:t.departure_time_local:""}_legDetailLabel(t){if("WALK"===t.mode||!t.departure_stop||!t.arrival_stop)return"";const e="nl"===this.language,i=(s=t.mode,_t.has(s));var s;const o=t.stop_count?` · ${t.stop_count} ${e?i?"station"+(1===t.stop_count?"":"s"):1===t.stop_count?"halte":"haltes":i?1===t.stop_count?"station":"stations":1===t.stop_count?"stop":"stops"}`:"";return`${this._stripCityPrefix(t.departure_stop)} → ${this._stripCityPrefix(t.arrival_stop)}${o}`}_stripCityPrefix(t){const e=t.indexOf(",");return-1===e?t:t.slice(e+1).trim()}_spanSeconds(t){if(t.departure_time&&t.arrival_time){const e=new Date(t.arrival_time).getTime()-new Date(t.departure_time).getTime();if(!Number.isNaN(e)&&e>=0)return Math.round(e/1e3)}return t.duration||0}};vt.styles=r`
    .bar {
      display: grid;
  ...
  `,t([pt({attribute:!1})],vt.prototype,"legs",void 0),t([pt({attribute:!1})],vt.prototype,"expanded",void 0),t([pt({attribute:!1})],vt.prototype,"language",void 0),vt=t([ct("google-transit-journey-bar")],vt);const yt=r`
  :host {
    display: block;
  }
...
`,wt=new Set(["BUS","TRAM","SUBWAY","LIGHT_RAIL"]);console.info("%c GOOGLE-TRANSIT-ROUTES-CARD %c v0.1.0 ","color: white; background: #1a73e8; font-weight: 700;","color: #1a73e8; background: white; font-weight: 700;");let bt=class extends at{constructor(){super(...arguments),this._expanded=new Set}static async getConfigElement(){return await Promise.resolve().then(function(){return Et}),document.createElement("google-transit-routes-card-editor")}static getStubConfig(){return{type:"custom:google-transit-routes-card",title:"Reistijden",entities:[],show_alternatives:!0,show_legs:!0,show_countdown:!0,refresh_interval:0,theme:"auto",compact:!1}}setConfig(t){if(!t.entities||!Array.isArray(t.entities))throw new Error("google-transit-routes-card: 'entities' is required");this._config={show_alternatives:!0,show_legs:!0,show_countdown:!0,refresh_interval:0,theme:"auto",compact:!1,...t}}getCardSize(){return 1+3*(this._config?.entities?.length||1)}connectedCallback(){super.connectedCallback(),this._scheduleRefresh()}disconnectedCallback(){super.disconnectedCallback(),this._clearRefresh()}updated(t){t.has("_config")&&(this._scheduleRefresh(),this._applyThemeClass())}_applyThemeClass(){this.classList.remove("force-light","force-dark"),"light"===this._config?.theme?this.classList.add("force-light"):"dark"===this._config?.theme&&this.classList.add("force-dark")}_clearRefresh(){void 0!==this._refreshTimer&&(window.clearInterval(this._refreshTimer),this._refreshTimer=void 0)}_scheduleRefresh(){this._clearRefresh();const t=1e3*(this._config?.refresh_interval??0);t&&(this._refreshTimer=window.setInterval(()=>this._refreshEntities(),t))}_refreshEntities(){if(!this.hass||!this._config)return;const t=this._config.entities.map(t=>t.entity).filter(Boolean);t.length&&this.hass.callService("homeassistant","update_entity",{entity_id:t})}_toggleExpanded(t){const e=new Set(this._expanded);e.has(t)?e.delete(t):e.add(t),this._expanded=e}render(){if(!this._config||!this.hass)return W``;const t=this._config.compact;return W`
    ...
    `}_renderRoute(t){const e=this.hass,i=this._config,s=t.entity?e.states[t.entity]:void 0,o=e.locale?.language||"en",n="nl"===o;if(!s)return W`
      ...
      `;const r=s.attributes,a=r.arrival_time,l=r.arrival_time_local,c=r.legs||[],h=r.alternative_routes||[],d=t.name||r.friendly_name||t.entity,p=this._expanded.has(t.entity),u=!!a&&new Date(a).getTime()<Date.now(),f=i.show_alternatives&&h.length>0;return W`
      <div
      ...
      >
        <div class="route-header">
          ...
          <span class="arrival">
            ${l?W`${n?"aankomst":"arrival"} ${l}`:n?"geen route gevonden":"no route found"}
          </span>
        </div>

        <div class="route-sub">
          ${i.show_countdown&&a?W`<google-transit-countdown
                .arrival=${a}
                .language=${o}
              ></google-transit-countdown>`:W`<span></span>`}
          ${f?W`<ha-icon
        ...
        ${i.show_legs&&c.length?W`<google-transit-journey-bar
              .legs=${c}
              .expanded=${p}
              .language=${o}
              ...
              <div class="alternatives">
                ...
                ${h.slice(0,3).map(t=>W`
                    <div class="alternative-row">
                      ...
                      <span class="alt-legs">
                        ${$t(t.legs||[]).map((t,e)=>W`${e>0?", ":""}${this._renderAltLeg(t)}`)}
                      </span>
                    </div>
                  `)}
              </div>
    ...
    `}_renderAltLeg(t){const e=mt(t.duration||0);if(wt.has(t.mode)&&t.line_name){const i=t.line_color||"var(--secondary-text-color, #727272)";return W`<span class="alt-leg"
        ><span class="alt-line-badge" style="background: ${i}"
          >${t.line_name}</span
        >
        (${e})</span
      >`}const i="WALK"===t.mode,s=gt[t.mode]??"mdi:map-marker-path";return W`<span class="alt-leg"
      ><ha-icon
        class=${i?"alt-walk-icon":"alt-mode-icon"}
        icon=${s}
      ></ha-icon>
      (${e})</span
    >`}};bt.styles=yt,t([pt({attribute:!1})],bt.prototype,"hass",void 0),t([ut()],bt.prototype,"_config",void 0),t([ut()],bt.prototype,"_expanded",void 0),bt=t([ct("google-transit-routes-card")],bt),window.customCards=window.customCards||[],window.customCards.push({type:"google-transit-routes-card",name:"Google Transit Routes Card",description:"Wall-mounted dashboard card showing saved Google Transit routes with live countdowns, journey bars and alternative departures.",preview:!0});const xt={show_alternatives:!0,show_legs:!0,show_countdown:!0,refresh_interval:0,theme:"auto",compact:!1};let At=class extends at{setConfig(t){this._config={...xt,...t}}_fireChanged(t){this._config=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}_valueChanged(t){if(!this._config)return;const e=t.target,i=e.configValue;if(!i)return;let s="checkbox"===e.type?e.checked:e.value;"refresh_interval"===i&&(s=Number(s)||0),this._fireChanged({...this._config,[i]:s})}_themeChanged(t){if(!this._config)return;const e=t.target.value;this._fireChanged({...this._config,theme:e})}_entityChanged(t,e,i){if(!this._config)return;const s=[...this._config.entities];s[t]={...s[t],[e]:i},this._fireChanged({...this._config,entities:s})}_addEntity(){if(!this._config)return;const t=[...this._config.entities||[],{entity:""}];this._fireChanged({...this._config,entities:t})}_removeEntity(t){if(!this._config)return;const e=this._config.entities.filter((e,i)=>i!==t);this._fireChanged({...this._config,entities:e})}render(){return this._config&&this.hass?W`
  ...
  `,t([pt({attribute:!1})],At.prototype,"hass",void 0),t([ut()],At.prototype,"_config",void 0),At=t([ct("google-transit-routes-card-editor")],At);var Et=Object.freeze({__proto__:null,get GoogleTransitRoutesCardEditor(){return At}});export{bt as GoogleTransitRoutesCard};
