 ...
 */function ut(t){return pt({...t,state:!0,attribute:!1})}let ft=class extends at{constructor(){super(...arguments),this.language="en",this._now=Date.now()}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=Date.now()},1e3)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(window.clearInterval(this._interval),this._interval=void 0)}_format(t){const e="nl"===this.language;if(t<=0)return e?"vertrokken":"departed";const i=Math.floor(t/3600),s=Math.floor(t%3600/60),o=e?"over":"in";return i>0?`${o} ${i}${e?"u":"h"} ${s}m`:`${o} ${s} min`}render(){if(!this.arrival)return B``;const t=new Date(this.arrival).getTime();if(Number.isNaN(t))return B``;const e=Math.round((t-this._now)/1e3);return B`<span class="countdown ${e<=0?"departed":""}"
  ...
  `,t([pt({attribute:!1})],ft.prototype,"arrival",void 0),t([pt({attribute:!1})],ft.prototype,"language",void 0),t([ut()],ft.prototype,"_now",void 0),ft=t([ct("google-transit-countdown")],ft);const gt={BUS:"mdi:bus",TRAM:"mdi:tram",SUBWAY:"mdi:subway",HEAVY_RAIL:"mdi:train",RAIL:"mdi:train",COMMUTER_TRAIN:"mdi:train",LIGHT_RAIL:"mdi:tram",FERRY:"mdi:ferry",WALK:"mdi:walk"};let _t=class extends at{constructor(){super(...arguments),this.legs=[]}render(){if(!this.legs?.length)return B``;const t=this.legs.reduce((t,e)=>t+(e.duration||0),0)||1,e=this.legs.filter(t=>"WALK"!==t.mode);return B`
      <div class="bar">
        ${this.legs.map(e=>{const i="WALK"===e.mode,s=(e.duration||0)/t*100,o=gt[e.mode]??"mdi:map-marker-path",n=e.line_color||"var(--secondary-text-color, #727272)";return B`
            <div
              class="segment ${i?"walk":"transit"}"
              style="flex-grow: ${Math.max(s,6)}; ${i?"":`background: ${n};`}"
              title=${e.line_full_name||e.mode}
            >
              ${i?B`<ha-icon icon="mdi:walk"></ha-icon>`:B`<ha-icon icon=${o}></ha-icon
      ...
      <div class="times">
        ${e.map(t=>B`<span
              >${t.departure_time_local} → ${t.arrival_time_local}</span
            >`)}
      </div>
    `}};_t.styles=r`
    .bar {
  ...
  `,t([pt({attribute:!1})],_t.prototype,"legs",void 0),_t=t([ct("google-transit-journey-bar")],_t);const mt=r`
  :host {
    display: block;
  }
...
`;console.info("%c GOOGLE-TRANSIT-ROUTES-CARD %c v0.1.0 ","color: white; background: #1a73e8; font-weight: 700;","color: #1a73e8; background: white; font-weight: 700;");let $t=class extends at{constructor(){super(...arguments),this._expanded=new Set}static async getConfigElement(){return await Promise.resolve().then(function(){return wt}),document.createElement("google-transit-routes-card-editor")}static getStubConfig(){return{type:"custom:google-transit-routes-card",title:"Reistijden",entities:[],show_alternatives:!0,show_legs:!0,show_countdown:!0,refresh_interval:0,theme:"auto",compact:!1}}setConfig(t){if(!t.entities||!Array.isArray(t.entities))throw new Error("google-transit-routes-card: 'entities' is required");this._config={show_alternatives:!0,show_legs:!0,show_countdown:!0,refresh_interval:0,theme:"auto",compact:!1,...t}}getCardSize(){return 1+3*(this._config?.entities?.length||1)}connectedCallback(){super.connectedCallback(),this._scheduleRefresh()}disconnectedCallback(){super.disconnectedCallback(),this._clearRefresh()}updated(t){t.has("_config")&&(this._scheduleRefresh(),this._applyThemeClass())}_applyThemeClass(){this.classList.remove("force-light","force-dark"),"light"===this._config?.theme?this.classList.add("force-light"):"dark"===this._config?.theme&&this.classList.add("force-dark")}_clearRefresh(){void 0!==this._refreshTimer&&(window.clearInterval(this._refreshTimer),this._refreshTimer=void 0)}_scheduleRefresh(){this._clearRefresh();const t=1e3*(this._config?.refresh_interval??0);t&&(this._refreshTimer=window.setInterval(()=>this._refreshEntities(),t))}_refreshEntities(){if(!this.hass||!this._config)return;const t=this._config.entities.map(t=>t.entity).filter(Boolean);t.length&&this.hass.callService("homeassistant","update_entity",{entity_id:t})}_toggleExpanded(t){const e=new Set(this._expanded);e.has(t)?e.delete(t):e.add(t),this._expanded=e}render(){if(!this._config||!this.hass)return B``;const t=this._config.compact;return B`
    ...
    `}_renderRoute(t){const e=this.hass,i=this._config,s=t.entity?e.states[t.entity]:void 0;if(!s)return B`
      ...
      `;const o=s.attributes,n=o.arrival_time,r=o.arrival_time_local,a=o.legs||[],h=o.alternative_routes||[],c=e.locale?.language||"en",l=this._expanded.has(t.entity),d=!!n&&new Date(n).getTime()<Date.now(),p=i.show_alternatives&&h.length>0;return B`
      <div
        class="route-row ${d?"departed":""}"
        @click=${p?()=>this._toggleExpanded(t.entity):void 0}
      >
        <div class="route-header">
          <ha-icon icon=${t.icon||"mdi:bus-clock"}></ha-icon>
          <span class="route-name">${t.name||t.entity}</span>
          <span class="arrival">
            ${r?B`arrival ${r}`:"no route found"}
          </span>
        </div>

        <div class="route-sub">
          ${i.show_countdown&&n?B`<google-transit-countdown
                .arrival=${n}
                .language=${c}
              ></google-transit-countdown>`:B`<span></span>`}
          ${p?B`<ha-icon
                class="expand-arrow ${l?"open":""}"
                icon="mdi:chevron-down"
              ></ha-icon>`:q}
        </div>

        ${i.show_legs&&a.length?B`<google-transit-journey-bar
              .legs=${a}
              ...
              <div class="alternatives">
                ${h.slice(0,3).map(t=>B`
                    <div class="alternative-row">
                      ...
                      <span class="alt-legs">
                        ${(t.legs||[]).filter(t=>"WALK"!==t.mode).map(t=>t.line_name).join(" + ")}
                      </span>
                    </div>
                  `)}
              </div>
    ...
    `}};$t.styles=mt,t([pt({attribute:!1})],$t.prototype,"hass",void 0),t([ut()],$t.prototype,"_config",void 0),t([ut()],$t.prototype,"_expanded",void 0),$t=t([ct("google-transit-routes-card")],$t),window.customCards=window.customCards||[],window.customCards.push({type:"google-transit-routes-card",name:"Google Transit Routes Card",description:"Wall-mounted dashboard card showing saved Google Transit routes with live countdowns, journey bars and alternative departures.",preview:!0});const vt={show_alternatives:!0,show_legs:!0,show_countdown:!0,refresh_interval:0,theme:"auto",compact:!1};let yt=class extends at{setConfig(t){this._config={...vt,...t}}_fireChanged(t){this._config=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t}}))}_valueChanged(t){if(!this._config)return;const e=t.target,i=e.configValue;if(!i)return;let s="checkbox"===e.type?e.checked:e.value;"refresh_interval"===i&&(s=Number(s)||0),this._fireChanged({...this._config,[i]:s})}_themeChanged(t){if(!this._config)return;const e=t.target.value;this._fireChanged({...this._config,theme:e})}_entityChanged(t,e,i){if(!this._config)return;const s=[...this._config.entities];s[t]={...s[t],[e]:i},this._fireChanged({...this._config,entities:s})}_addEntity(){if(!this._config)return;const t=[...this._config.entities||[],{entity:""}];this._fireChanged({...this._config,entities:t})}_removeEntity(t){if(!this._config)return;const e=this._config.entities.filter((e,i)=>i!==t);this._fireChanged({...this._config,entities:e})}render(){return this._config&&this.hass?B`
  ...
  `,t([pt({attribute:!1})],yt.prototype,"hass",void 0),t([ut()],yt.prototype,"_config",void 0),yt=t([ct("google-transit-routes-card-editor")],yt);var wt=Object.freeze({__proto__:null,get GoogleTransitRoutesCardEditor(){return yt}});export{$t as GoogleTransitRoutesCard};
